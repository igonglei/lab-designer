...
(function($, window, document, undefined) {
    ...
    plugin = {
        ...
        newGraph: function(instance) {
            var editor = instance.editor = new mxEditor(), graph = instance.graph = editor.graph, $el = $(instance.dom).empty(), $designer = $('<div class="' + classList.designer + '"></div>').appendTo($el);
        ...
        setBackground: function(instance, reset) {
            var opts = instance.options, graph = instance.graph, src = instance.path.background, width = opts.width, height = opts.height;
        ...
        initFilePath: function(instance) {
            var opts = instance.options, dataDir = utils.tpl("{root}{dataDir}{lab}/", opts);
            instance.path = {
        ...
        loadData: function(instance, isbak) {
            var self = this, opts = instance.options, url = opts.url, onLoad = function(data) {
                if (!data || !data.vertex) {
        ...
        fillGraph: function(instance, cells, selected) {
            var graph = instance.graph, model = graph.getModel(), parent = graph.getDefaultParent(), vertex = cells || instance.data.vertex, self = this;
            model.beginUpdate();
            try {
        ...
        save: function(instance) {
            ...
            }), usedCab = this.getUsedCells(instance, vertexs).length, speCabArr = this.getSpecialCab(instance, vertexs);
            var self = this;
        ...
        setEditorToolbar: function(instance) {
            var $el = $('<ul class="' + classList.eToolbar + '"></ul>').prependTo($(instance.dom)), self = this;
            $.each(data.etools, function(i, v) {
                var type = v.type, tpl = type === classList.separator ? templates.separator : templates.etools, key = v.key || "", keyText = key && " (" + key + ")";
        ...
        setEditorToolbox: function(instance) {
            var $el = $('<div class="' + classList.eToolbox + '"></div>').appendTo($(instance.dom)), $box = $('<ul class="' + classList.eBox + '"></ul>').appendTo($el), self = this, opts = instance.options;
            this.setToggleIcon(instance, $box, "left-hide", "right-show");
            var toolbox = {};
            $.each(styles, function(i, v) {
                var type = v.type, tools = toolbox[type] || [];
                tools.push(v);
                toolbox[type] = tools;
            });
        ...
        setToggleIcon: function(instance, $box, hideCls, showCls) {
            var self = this, $toggle = $(utils.tpl(templates.toggle_icon, {
                cls: hideCls,
        ...
        makeDraggable: function(img, instance) {
            ...
            var graph = instance.graph, dragImage = img.cloneNode(true), self = this;
        ...
        setContextMenu: function(instance) {
            var opts = instance.options, contextMenu = opts.contextMenu, self = this, graph = instance.graph;
            ...
            graph.popupMenuHandler.factoryMethod = typeof contextMenu === "function" ? function(menu, cell, evt) {
                ...
                if (opts.editorMode) {
                    var cells = graph.getSelectionCells(), menus = cells.length === 0 ? data.emenus : data.emenus4cell;
                    $.each(menus, function(i, v) {
                        var type = v.type, onclick = v.onclick;
        ...
        pasteHere: function(instance) {
            ...
            try {
                ...
                var t = graph.view.translate, s = graph.view.scale, dx = t.x, dy = t.y, x = Math.round(graph.snap(graph.popupMenuHandler.triggerX / s - dx)), y = Math.round(graph.snap(graph.popupMenuHandler.triggerY / s - dy));
        ...
        setShortcuts: function(instance) {
            var graph = instance.graph, keyHandler = new mxKeyHandler(graph);
        ...
        setArrowEvent: function(instance) {
            var graph = instance.graph, keyHandler = instance.keyHandler, opts = instance.options, moveStepSize = opts.moveStepSize, resizeStepSize = opts.resizeStepSize, ARROW = {
                LEFT: 37,
                UP: 38,
                RIGHT: 39,
                DOWN: 40
            }, queue = [], thread = null, nudge = function(keyCode, stepSize, resize) {
                queue.push(function() {
                    if (resize) {
                        graph.getModel().beginUpdate();
                        try {
        ...
        setTooltip: function(instance) {
            var graph = instance.graph, opts = instance.options, tooltip = opts.tooltip, self = this;
            graph.getTooltipForCell = typeof tooltip === "function" ? function(cell) {
                ...
                var value = cell.value, target = instance.dom.id + "-" + value.name, cabData = {
                ...
                self.getCabInfo(instance, value.name, cell.style).done(function(data) {
                    var cabInfo = instance.cabInfo, devices = cabInfo.deviceList, cabInfo = cabInfo.uNum[0] || {
                        allU: 0,
                        usedU: 0
                    }, device_html = "";
        ...
        setLengend: function(instance) {
            var $ul = $('<ul class="' + classList.lengend + '"></ul>').appendTo($(instance.dom)), self = this;
            $.each(styles, function(i, v) {
                if (v.lengend) {
        ...
        setToolbar: function(instance) {
            var $div = $('<div class="' + classList.toolbar + '"></div>').prependTo($(instance.dom)), $tools = $('<ul class="' + classList.tools + '"></ul>').appendTo($div), $tip = $(utils.tpl('<ul class="{cls}"><li>{text}(%)</li></ul>', {
                cls: classList.tip,
                text: this.getText(instance, "capacity")
            })).appendTo($div), self = this;
        ...
        setEditorPropbox: function(instance) {
            var $el = $('<div class="' + classList.ePropbox + '"></div>').appendTo($(instance.dom)), $div = $('<div class="' + classList.eBox + '"></div>').appendTo($el);
        ...
        initLabPropbox: function(instance) {
            var $el = $(instance.dom), $div = $el.find("." + classList.ePropbox4Lab);
            ...
            var $propBody = $('<div class="lde-editor-propbox-body"></div>').appendTo($div), $list = $('<ul class="' + classList.ePropboxLabList + '"></ul>').appendTo($propBody), opts = instance.options, selectedCls = "selected", self = this, reload = function($li) {
        ...
        initCabPropbox: function(instance, cell) {
            var $el = $(instance.dom), $div = $el.find("." + classList.ePropbox4Cab), show = function() {
                $div.show().prev().hide();
            }, selectedCls = classList.selectedCls, name = cell.value && cell.value.name, groupName = this.getGroupName(name);
            ...
            var $propBody = $('<div class="lde-editor-propbox-body"></div>').appendTo($div), $list = $('<ul class="' + classList.ePropboxCabList + '"></ul>').appendTo($propBody), opts = instance.options, self = this;
        ...
        loadCabList: function(instance, cell, cabType, groupName, selectedCls) {
            ...
            var $el = $(instance.dom), $div = $el.find("." + classList.ePropbox4Cab), $propBody = $('<div class="lde-editor-propbox-body"></div>').appendTo($div), name = cell.value && cell.value.name, $list = $('<ul class="' + classList.ePropboxCabList + '"></ul>').appendTo($propBody);
        ...
        groupCabs: function(instance, cabType) {
            var cabList = instance.cabList, groups = {}, self = this;
        ...
        setCheckCellOverlay: function(instance, cell) {
            var graph = instance.graph, opts = instance.options, overlaySize = opts.checkOverlaySize, overlayImg = new mxImage(instance.path.imgPath + opts.checkOverlayImage, overlaySize, overlaySize), overlay = new mxCellOverlay(overlayImg, null, "center", "middle", null, "default");
        ...
        setRateCellOverlay: function(instance, cell, rate) {
            var graph = instance.graph, imgPath = instance.path.imgPath, opts = instance.options, width = opts.rateOverlaySize, rateImages = opts.rateOverlayImage, defaultCabSize = styles[0].width;
            var add = function(cell, rate) {
                ...
                var direction = mxUtils.getValue(graph.view.getState(cell).style, mxConstants.STYLE_DIRECTION, "east"), overlayImg, overlay, tw, th, src, x, y, diff = 8, cw = cell.geometry.width, ch = cell.geometry.height, per;
            ...
            var usedCells = this.getUsedCells(instance), cabUCountList = instance.cabUCountList || [];
        ...
        getText: function(instance, type) {
            var text = texts[type], lang = instance.options.lang;
        ...
        getLabList: function(instance) {
            var opts = instance.options, self = this, url = opts.labListAction;
            if (!url) {
        ...
        getCabList: function(instance) {
            var opts = instance.options, self = this;
            self.infoMsg(instance, self.getText(instance, "loading"));
        ...
        getCabInfo: function(instance, cabNo, cabType) {
            var opts = instance.options, curLabId = opts.lab, self = this, url = opts.cabInfoAction, IsSpcCab = cabType === "cab-dev" || cabType === "cab-iond" || cabType === "cab-server";
            //是否是特殊机柜（U位占用率为百分百）
            if (!url) {
        ...
        getCabUCount: function(instance) {
            var opts = instance.options, self = this, url = opts.cabUCountAction;
            if (!url) {
                return $.ajax().done(function() {
        ...
        modal: function(instance, onOpen) {
            var $mask = $('<div class="' + classList.modalMask + '" id="' + instance.dom.id + '-lde-mask"></div>').appendTo($(document.body)), $modal = $('<div class="' + classList.modal + '"><div class="' + classList.modalBody + '"></div></div>').appendTo($mask), $close = $('<div class="lde-designer-modal-close">×</div>').appendTo($modal);
            if (onOpen) {
                onOpen.call($mask);
            }
        ...
        confirm: function(instance, msg, onOK) {
            ...
            this.modal(instance, function() {
                var $mask = this, $mbody = $mask.find("." + classList.modalBody);
        ...
        exportImage: function(instance) {
            var graph = instance.graph, opts = instance.options, lab = opts.lab, url = opts.exportAction;
            if (!url) {
                return;
            }
            var scale = 1;
        ...
        show: function(instance) {
            var graph = instance.graph, opts = instance.options, bounds = graph.getGraphBounds(), wnd = window.open(), doc = wnd.document;
        ...
        rotateCells: function(instance) {
            ...
            var opts = instance.options, vertexHandlerCreateSizerShape = mxVertexHandler.prototype.createSizerShape;
